...
class KGT5_Model(pl.LightningModule):
    ...
    def evaluate(self, batch, mode='val'):
        ...
        query_frequency = len(self.dataset.train_filter_dict[query])
        if len(true_pos) == 0:
            ...
            if query_frequency > 10:
                ranks_dict["frequent"].append(self.dataset.num_entities)
            else:
                ranks_dict["infrequent"].append(self.dataset.num_entities)
        ...
        if query_frequency > 10:
            ranks_dict["frequent"].append(rank + ties // 2 + 1)
        else:
            ranks_dict["infrequent"].append(rank + ties // 2 + 1)
        return ranks_dict
    ...
    def metric_aggregation(self, ranks_dicts):
        ...
        frequent_ranks = np.array([rd["frequent"] for rd in ranks_dicts if len(rd["frequent"]) > 0]).squeeze()
        infrequent_ranks = np.array([rd["infrequent"] for rd in ranks_dicts if len(rd["infrequent"]) > 0]).squeeze()
        for r, suffix in zip([ranks, head_ranks, tail_ranks, frequent_ranks, infrequent_ranks], ["", "_head", "_tail", "_frequent", "_infrequent"]):
            if len(r) != 0:
